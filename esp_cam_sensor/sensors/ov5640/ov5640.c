...
static const esp_cam_sensor_isp_info_t ov5640_isp_info_mipi[] = {
    /* For MIPI */
    {
        .isp_v1_info =
            {
        ...
        .mipi_info =
            {
                ...
                .lane_num = 2,
                .line_sync_en =
                    CONFIG_CAMERA_OV5640_CSI_LINESYNC_ENABLE ? true : false,
            },
        .reserved = NULL,
    },
    {
        .name = "MIPI_2lane_24Minput_RAW10_1280x720_14fps",
        .format = ESP_CAM_SENSOR_PIXFORMAT_RAW10,
        .port = ESP_CAM_SENSOR_MIPI_CSI,
        .xclk = 24000000,
        .width = 1280,
        .height = 720,
        .regs = ov5640_MIPI_2lane_raw10b_720p_14fps,
        .regs_size = ARRAY_SIZE(ov5640_MIPI_2lane_raw10b_720p_14fps),
        .fps = 14,
        .isp_info = &ov5640_isp_info_mipi[0],
        .mipi_info =
            {
                .mipi_clk = OV5640_LINE_RATE_16BITS_1280x720_14FPS,
                .lane_num = 2,
...
static esp_err_t ov5640_read(esp_sccb_io_handle_t sccb_handle, uint16_t reg,
                             uint8_t *read_buf) {
  return esp_sccb_transmit_receive_reg_a16v8(sccb_handle, reg, read_buf);
}

static esp_err_t ov5640_write(esp_sccb_io_handle_t sccb_handle, uint16_t reg,
                              uint8_t data) {
  return esp_sccb_transmit_reg_a16v8(sccb_handle, reg, data);
}

/* write a array of registers  */
static esp_err_t ov5640_write_array(esp_sccb_io_handle_t sccb_handle,
                                    const ov5640_reginfo_t *regarray) {
  int i = 0;
...
static esp_err_t ov5640_set_reg_bits(esp_sccb_io_handle_t sccb_handle,
...
static esp_err_t ov5640_set_test_pattern(esp_cam_sensor_device_t *dev,
                                         int enable) {
...
static esp_err_t ov5640_soft_reset(esp_cam_sensor_device_t *dev) {
  esp_err_t ret =
      ov5640_set_reg_bits(dev->sccb_handle, OV5640_REG_SYS_CTRL0, 7, 1, 0x01);
  delay_ms(5);
  return ret;
}

static esp_err_t ov5640_get_sensor_id(esp_cam_sensor_device_t *dev,
...
static esp_err_t ov5640_set_stream_dvp(esp_cam_sensor_device_t *dev, bool on) {
  return ov5640_write(dev->sccb_handle, OV5640_REG_SYS_CTRL0,
                      on ? OV5640_SOFT_POWER_DOWN_DIS
...
static esp_err_t ov5640_set_stream_mipi(esp_cam_sensor_device_t *dev, bool on) {
  ...
  ret =
      ov5640_write(dev->sccb_handle, OV5640_REG_SYS_CTRL0,
...
static esp_err_t ov5640_query_para_desc(esp_cam_sensor_device_t *dev,
...
static esp_err_t ov5640_get_para_value(esp_cam_sensor_device_t *dev,
                                       uint32_t id, void *arg, size_t size) {
  return ESP_ERR_NOT_SUPPORTED;
}

static esp_err_t ov5640_set_para_value(esp_cam_sensor_device_t *dev,
                                       uint32_t id, const void *arg,
                                       size_t size) {
  esp_err_t ret = ESP_OK;
...
static esp_err_t
ov5640_query_support_formats(esp_cam_sensor_device_t *dev,
...
static esp_err_t
ov5640_query_support_capability(esp_cam_sensor_device_t *dev,
...
static esp_err_t ov5640_set_format(esp_cam_sensor_device_t *dev,
                                   const esp_cam_sensor_format_t *format) {
  ESP_CAM_SENSOR_NULL_POINTER_CHECK(TAG, dev);

  esp_err_t ret = ESP_OK;
  ov5640_reginfo_t *reset_regs_list = NULL;
  /* Depending on the interface type, an available configuration is
  automatically loaded. You can set the output format of the sensor without
  using query_format().*/
  if (format == NULL) {
#if CONFIG_SOC_MIPI_CSI_SUPPORTED
    if (dev->sensor_port == ESP_CAM_SENSOR_MIPI_CSI) {
      reset_regs_list = (ov5640_reginfo_t *)ov5640_mipi_reset_regs;
      format = &ov5640_format_info_mipi
                   [CONFIG_CAMERA_OV5640_MIPI_IF_FORMAT_INDEX_DEFAULT];
    }
#endif
#if CONFIG_SOC_LCDCAM_CAM_SUPPORTED
    if (dev->sensor_port == ESP_CAM_SENSOR_DVP) {
      reset_regs_list = (ov5640_reginfo_t *)ov5640_dvp_reset_regs;
      format = &ov5640_format_info_dvp
                   [CONFIG_CAMERA_OV5640_DVP_IF_FORMAT_INDEX_DEFAULT];
    }
#endif
  }
  ...
  ret = ov5640_write_array(dev->sccb_handle,
                           (const ov5640_reginfo_t *)format->regs);
  ESP_RETURN_ON_FALSE(ret == ESP_OK, ESP_CAM_SENSOR_ERR_FAILED_SET_FORMAT, TAG,
                      "write format regs failed");
...
static esp_err_t ov5640_get_format(esp_cam_sensor_device_t *dev,
...
static esp_err_t ov5640_priv_ioctl(esp_cam_sensor_device_t *dev, uint32_t cmd,
                                   void *arg) {
  esp_err_t ret = ESP_OK;
  uint8_t regval;
  ...
  case ESP_CAM_SENSOR_IOC_S_REG:
    ...
    ret =
        ov5640_write(dev->sccb_handle, sensor_reg->regaddr, sensor_reg->value);
    break;
...
esp_cam_sensor_device_t *ov5640_detect(esp_cam_sensor_config_t *config) {
  ...
  if (config->sensor_port == ESP_CAM_SENSOR_MIPI_CSI) {
    dev->cur_format = &ov5640_format_info_mipi
                          [CONFIG_CAMERA_OV5640_MIPI_IF_FORMAT_INDEX_DEFAULT];
  }
#endif
  ...
  if (config->sensor_port == ESP_CAM_SENSOR_DVP) {
    dev->cur_format = &ov5640_format_info_dvp
                          [CONFIG_CAMERA_OV5640_DVP_IF_FORMAT_INDEX_DEFAULT];
  }
#endif
...
ESP_CAM_SENSOR_DETECT_FN(ov5640_detect, ESP_CAM_SENSOR_MIPI_CSI,
                         OV5640_SCCB_ADDR) {
